...
<ul v-if="cart.length">
    <li v-for="(prod, index) in cart" :key="prod.id" > <b>НАЗВАНИЕ:</b> {{ prod.name }} <br> <b>ОПИСАНИЕ:</b> {{ prod.description }} <br> <b>ID ТОВАРА:</b> {{ prod.product_id }}  <br> <b>Колличество:</b> {{ prod.count }}<br> <b>ЦЕНА:</b> {{ prod.price }}<br>
